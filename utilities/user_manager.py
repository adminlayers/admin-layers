...
class UserManagerUtility(BaseUtility):
    ...
    def _action_bar(self) -> None:
        ...
        if c3.button("Queues", use_container_width=True, key="um_ab_queues"):
            ...
            st.rerun()
        if c4.button("Edit", use_container_width=True, key="um_ab_edit"):
            self.set_state("page", "edit")
            st.rerun()
        if c5.button("Refresh", use_container_width=True, key="um_ab_ref"):
    ...
    def _page_list(self) -> None:
        ...
        st.caption(
            f"Showing {len(df)} of {total} users (Page {page_number} of {page_count})"
        )
        ...
        options = {
            f"{u.get('name', '?')} ({u.get('email', '?')})": u.get("id")
            for u in filtered
        }
        if options:
            chosen = st.selectbox(
    ...
    def _page_search(self) -> None:
        ...
        if results:
            ...
            for u in results[:10]:
                ...
                if st.button(
                    label, key=f"um_pick_{u.get('id')}", use_container_width=True
                ):
    ...
    def _page_detail(self) -> None:
        ...
        st.caption(
            "Expanded profile view with quick actions to manage group, skill, and queue access."
        )
        ...
        fields = [
            ...
            (
                "Manager",
                info.get("manager", {}).get("name") if info.get("manager") else None,
            ),
            (
                "Division",
                info.get("division", {}).get("name") if info.get("division") else None,
            ),
        ]
        ...
        tab_group, tab_skill, tab_queue = st.tabs(
            ["Add to Group", "Assign Skill", "Add to Queue"]
        )
        ...
        with tab_group:
            ...
            if group_options:
                ...
                selected_group = st.selectbox(
                    "Select group", list(group_map.keys()), key="um_quick_group_pick"
                )
                confirm = st.checkbox(
                    "I confirm adding this user to the group",
                    key="um_quick_group_confirm",
                )
                if st.button(
                    "Add to Group",
                    type="primary",
                    disabled=not confirm,
                    key="um_quick_group_btn",
                ):
                    resp = self.api.groups.add_members(
                        group_map[selected_group], [info["id"]]
                    )
                    if resp.success:
        ...
        with tab_skill:
            ...
            if skills:
                ...
                selected_skill = st.selectbox(
                    "Select skill", list(skill_map.keys()), key="um_quick_skill_pick"
                )
                proficiency = st.slider(
                    "Proficiency", 0.0, 5.0, 3.0, 0.5, key="um_quick_prof"
                )
                confirm = st.checkbox(
                    "I confirm assigning this skill", key="um_quick_skill_confirm"
                )
                if st.button(
                    "Assign Skill",
                    type="primary",
                    disabled=not confirm,
                    key="um_quick_skill_btn",
                ):
                    resp = self.api.routing.add_user_skill(
                        info["id"], skill_map[selected_skill], proficiency
                    )
                    if resp.success:
        ...
        with tab_queue:
            queue_query = st.text_input(
                "Search queues",
                placeholder="Start typing a queue name...",
                key="um_quick_queue",
            )
            queue_options = []
            if queue_query:
            ...
            if queue_options:
                ...
                selected_queue = st.selectbox(
                    "Select queue", list(queue_map.keys()), key="um_quick_queue_pick"
                )
                confirm = st.checkbox(
                    "I confirm adding this user to the queue",
                    key="um_quick_queue_confirm",
                )
                if st.button(
                    "Add to Queue",
                    type="primary",
                    disabled=not confirm,
                    key="um_quick_queue_btn",
                ):
                    resp = self.api.queues.add_members(
                        queue_map[selected_queue], [info["id"]]
                    )
                    if resp.success:
        ...
        if groups is None:
            with st.spinner("Loading user groups..."):
                ...
                if resp.success:
                    data = resp.data
                    groups = (
                        data.get("entities", data) if isinstance(data, dict) else data
                    )
